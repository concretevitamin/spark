        ...
        q"""
          ...
          var $primitiveTerm: String =
            if($nullTerm) {
              ...
              ${stringEval.primitiveTerm}.substring(${startEval.primitiveTerm}, if(${endEval.primitiveTerm} <= ${stringEval.primitiveTerm}.length) ${endEval.primitiveTerm} else ${stringEval.primitiveTerm}.length)
            }
        """.children
    }
...
object GenerateCondition extends CodeGenerator {
  ...
  def createCondition(condition: Expression): ((Row) => Boolean) = {
    ...
    logger.debug(s"Generated condition '$condition':\n$code")
...
object GenerateMutableProjection extends CodeGenerator {
  ...
  def createProjection(expressions: Seq[Expression]): (() => MutableProjection) = {
    ...
    logger.debug(s"code for ${expressions.mkString(",")}:\n$code")
...
object GenerateProjection extends CodeGenerator {
  ...
  def createProjection(expressions: Seq[Expression]): Projection = {
    ...
    logger.debug(s"MutableRow, initExprs: ${expressions.mkString(",")} code:\n${toolBox.typeCheck(code)}")
